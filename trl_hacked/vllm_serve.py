...
def llm_worker(
    ...
    llm = LLM(
        ...
        worker_extension_cls="trl_hacked.vllm_serve.WeightSyncWorkerExtension",
...
def main(script_args: ScriptArguments):
    ...
    class GenerateRequest(BaseModel):
        prompts: Union[list[str], list[dict[str, list[int]]]]
        n: int = 1
    ...
    class GenerateResponse(BaseModel):
        ...
        logprobs: Optional[list[list[dict[int, float]]]]
    ...
    async def generate(request: GenerateRequest):
        ...
        Args:
            request (`GenerateRequest`):
                - `prompts` (list of `str`): A list of prompts for the model to generate completions.

        Returns:
            `GenerateResponse`:
                - `completion_ids` (list of list of `int`): A list of lists of token IDs for each generated completion.
                - `logprobs` (optional, list of list of `dict[int, float]`): A list of lists of dicts mapping token IDs to their log probabilities at each generation step.
        ...
        for connection, prompts in zip(connections, chunked_prompts):
            ...
            connection.send({"type": "call", "method": "generate", "args": args, "kwargs": kwargs})
        ...
        response = {"completion_ids": completion_ids, "logprobs": []}
        if request.logprobs is not None:
            logprobs = [
                [{token_id: logprob.logprob for token_id, logprob in logprobs.items()} for logprobs in output.logprobs]
    ...
    async def embed(request: EmbedRequest):
        """
        Computes embeddings for the provided prompts.

        Args:
            request (`EmbedRequest`):
                - `prompts` (list of `str`): A list of prompts for the model to generate embeddings.

        Returns:
            `EmbedResponse`:
                - `embeddings` (list of list of `float`): A list of lists of embeddings for each prompt.
        ...
        for connection, prompts in zip(connections, chunked_prompts):
            ...
            if not prompts:
                prompts = ["<placeholder>"]
            args = (prompts,)
            connection.send({"type": "call", "method": "embed", "args": args})
        ...
        # Flatten and combine all results
        all_outputs = list(chain.from_iterable(all_outputs))  # from list of list to single list
        embeddings = [list(outputs.outputs.embedding) for outputs in all_outputs]
